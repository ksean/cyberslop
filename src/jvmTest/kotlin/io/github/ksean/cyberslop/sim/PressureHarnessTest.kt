...
class PressureHarnessTest {
    ...
    fun `a mini-boss award is pinned before its collecting jump`() {
        ...
        live.placeAt(sim.player.centre(Physics.Default) + Vec2(0.0, live.height / 2.0))
        live.fight.damage(live.spec.maxHealth)

        sim.tick(InputFrame())

        assertTrue(live.fight.defeated, "fixture: the mini-boss did not die")
        assertTrue(sim.items.any { it.isGuaranteedEquipment }, "fixture: the raised award was collected on the death tick")
        ...
        assertTrue(sim.items.none { it.isGuaranteedEquipment }, "the collecting jump left the award behind")
