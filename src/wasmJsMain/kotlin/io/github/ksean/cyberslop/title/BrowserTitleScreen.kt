...
internal fun renderTitleScreen(root: HTMLElement, state: TitleScreenState) {
    root.textContent = ""
    root.appendChild(titleElement(state.title))
    state.actions.forEach { action ->
        root.appendChild(actionButton(action))
    }
}
...
private fun actionButton(action: TitleScreenAction): HTMLButtonElement =
