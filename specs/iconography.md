...
The intended silhouette of each of the forty-four item icons (PROD-049). Whether an icon is
recognisable is a human judgement made against the icon sheet; this catalog is what the sheet is
judged against. Icons are composed from shared parts (`grip`, `stock`, `muzzle`, `ring`,
...
| Static Lash | a taper from Slab handle through Line body to Hair tip, pommel and a spark dot |
...
| Tenement Nailgun | boxy body, stubby barrel, nail strip on top with the nails in it |
...
| Ghostwire Tether | an anchor plate, a slack wire, and a hook large enough to be the subject |
...
## Powerups — every one inside a module casing
